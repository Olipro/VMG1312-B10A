...
static int open_tcp_socket(ares_channel channel, struct server_state *server)
{
  ...
  /*ZyXEL Multi-interface*///Michael.20151130.001(a): Add by merging from GIT trunk (40ad7932a) done by Steve to make Ares Lib's operation socket support setting Packet Mark value to support Multi-Interface Binding Policy Route for the DNS query.
  //printf("Ares IfMark:%x\n", channel->ifMark);
  if(setsockopt(s, SOL_SOCKET, SO_MARK, &channel->ifMark, sizeof(channel->ifMark)) == -1)
    {
      printf("%s(): <<Error>> Ares setsockopt(SO_MARK) for 'channel->ifMark' = %d (0x%08X) FAILed!!\n", __FUNCTION__, channel->ifMark);
    }
