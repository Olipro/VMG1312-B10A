...
//    struct pppoe_tag *tags = NULL;
    ...
    //cmsLog_error("SEND PADI...\n"); //ZyXEL Ryan add to debug
    
/* __ZyXEL__ ,Ryan */
//send a PADT packet before establish a new PPP connetion
	   if(strlen(oldsession) > 0){
	   	
	   	   //cmsLog_error("send a PADT packet before PADI\n");
	   	   //cmsLog_error("SID=%d\n",ses->sp.sa_addr.pppoe.sid);
           //cmsLog_error("Before send PADT\n");
	       session_disconnect(ses);
           //cmsLog_error("After send PADT\n");
	   	}
...
int is_recv_padt(void)
{
   ...
   if (ret < 0 && errno != EINTR)
      ...
      if ( rcv_packet.hdr->code == PADT_CODE && rcv_packet.hdr->sid == old_ses.sp.sa_addr.pppoe.sid) {
         //cmsLog_error("%02x%02x%02x%02x%02x%02x/%04x OLD %04x\n",  rcv_packet.addr.sll_addr[0],  rcv_packet.addr.sll_addr[1],  rcv_packet.addr.sll_addr[2],
         //         rcv_packet.addr.sll_addr[3],  rcv_packet.addr.sll_addr[4],  rcv_packet.addr.sll_addr[5], rcv_packet.hdr->sid, old_ses.sp.sa_addr.pppoe.sid);
         return 1;
      }
   }
   return 0;
