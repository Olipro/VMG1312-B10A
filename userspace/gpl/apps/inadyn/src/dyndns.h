...
/* dyndns.org specific update address format */	
#define DYNDNS_GET_MY_IP_HTTP_REQUEST_FORMAT_Thailand \
	"GET /nic/update?" \
		"system=%s&" \
		"hostname=%s&" \
		"myip=%s&" \
...
#define GENERIC_NOIP_AUTH_MY_IP_REQUEST_FORMAT_Thailand \
	"GET /update.php?username=%s&pass=%s&host=%s&ip=%s " \
	"HTTP/1.0\r\n" \
	"Host: %s\r\n" \
	"User-Agent: "DYNDNS_AGENT_NAME " " DYNDNS_EMAIL_ADDR"\r\n\r\n"
