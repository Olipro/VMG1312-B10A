...
static int get_req_for_dyndns_server_Thailand(DYN_DNS_CLIENT *this, int nr, DYNDNS_SYSTEM *p_sys_info);
...
static int get_req_for_noip_http_dns_server_Thailand(DYN_DNS_CLIENT *p_self, int cnt,  DYNDNS_SYSTEM *p_sys_info);

static BOOL is_dyndns_server_rsp_ok( DYN_DNS_CLIENT *p_self, char*p_rsp, char* p_ok_string);
static BOOL is_dyndns_server_rsp_ok_for_noip( DYN_DNS_CLIENT *p_self, char*p_rsp, char* p_ok_string);
...
DYNDNS_SYSTEM_INFO_Thailand dns_system_table_Thailand[] = 
{ 
    {DYNDNS_DEFAULT, 
        {"default@dyndns.org", &dyndns_org_dynamic, 
            (DNS_SYSTEM_SRV_RESPONSE_OK_FUNC)is_dyndns_server_rsp_ok, 
            (DNS_SYSTEM_REQUEST_FUNC)get_req_for_dyndns_server_Thailand,
    ...
    {NOIP_DEFAULT, 
        {"default@no-ip.com", NULL,  
            (DNS_SYSTEM_SRV_RESPONSE_OK_FUNC)is_dyndns_server_rsp_ok_for_noip, 
            (DNS_SYSTEM_REQUEST_FUNC) get_req_for_noip_http_dns_server_Thailand,
#if 1 /* Support DDNS with inadyn, __FTTB7-CHT__, MitraStar chiahsin, 20120703 */
            DYNDNS_MY_IP_SERVER, "/",
#else
            "ip1.dynupdate.no-ip.com", "/", 
#endif            
			"dynupdate.no-ip.com", "/update.php?hostname=", ""}},		
...
CmsRet getCustomerName(GetCustomername *CustomerName)
{
    ...
    msg->type = CMS_MSG_GET_CUSTOMER_NAME;
    msg->src = EID_DDNSD;
    msg->dst = EID_SSK;
    msg->flags_request = 1;
    ...
    if ((ret = cmsMsg_send(msgHandle, msg)) != CMSRET_SUCCESS) {
        ...
        if ((ret = cmsMsg_receiveWithTimeout(msgHandle, &msg, 1000)) != CMSRET_SUCCESS) {
            ...
            switch(msg->type)         {
                case CMS_MSG_GET_CUSTOMER_NAME:
                    if(msg->dataLength != sizeof(GetCustomername)) {
                        cmsLog_error("Invalid Data Length: received %d != expected %d", msg->dataLength, sizeof(GetCustomername));
                        ret = CMSRET_INTERNAL_ERROR;
                        break;
                    }
                    msgBuf = (GetCustomername *)(msg + 1);
                    memcpy(CustomerName,msgBuf,sizeof(GetCustomername));
                    break;
                default:
                    cmsLog_error("Invalid message type (%x), expects CMS_MSG_GET_CUSTOMER_NAME (%x)", (unsigned int)msg->type,CMS_MSG_GET_CUSTOMER_NAME);
                    ret = CMSRET_INTERNAL_ERROR;
            }
            CMSMEM_FREE_BUF_AND_NULL_PTR(msg);
        } /* else rx ok */
...
static DYNDNS_SYSTEM* get_dns_system_by_id(DYNDNS_SYSTEM_ID id)
{
	...
	if (getCustomerName(&CustomerName) == CMSRET_SUCCESS)
    {
		strcpy(tmp,CustomerName.customername);
	}
	//fprintf(stderr,"Debug:In get_dns_system_by_id tmp=%s\n",tmp);
	
	if ( !strcmp(tmp,"Thailand") ){	
			//fprintf(stderr,"Debug:In get_dns_system_by_id case:Thailand\n"); 
			
			DYNDNS_SYSTEM_INFO_Thailand *it;
			for (it = dns_system_table_Thailand; it->id != LAST_DNS_SYSTEM; ++it)
			{
				if (it->id == id)
				{
					return &it->system;
...
DYNDNS_SYSTEM_INFO_Thailand* get_dyndns_system_table_Thailand(void)
{
    return dns_system_table_Thailand;
}
...
static int get_req_for_dyndns_server_Thailand(DYN_DNS_CLIENT *p_self, int cnt,DYNDNS_SYSTEM *p_sys_info)
{	
    DYNDNS_ORG_SPECIFIC_DATA *p_dyndns_specific = 
		(DYNDNS_ORG_SPECIFIC_DATA*) p_sys_info->p_specific_data;
	return sprintf(p_self->p_req_buffer, DYNDNS_GET_MY_IP_HTTP_REQUEST_FORMAT_Thailand,
        //p_self->info.dyndns_server_name.name,
		//p_self->info.dyndns_server_url,
		p_dyndns_specific->p_system,
		p_self->alias_info.names[cnt].name,
...
{
	(void)p_sys_info;
	return sprintf(p_self->p_req_buffer, GENERIC_NOIP_AUTH_MY_IP_REQUEST_FORMAT_Thailand,
      //  p_self->info.dyndns_server_name.name,//no-ip.com
		//p_self->info.dyndns_server_url,	
...
static BOOL is_dyndns_server_rsp_ok_for_noip( DYN_DNS_CLIENT *p_self, char*p_rsp, char* p_ok_string)
{
	(void) p_ok_string;
	  ...
	  return ( (strstr(p_rsp, "status=1") != NULL) ||
             (strstr(p_rsp, "status=0") != NULL) );
...
static RC_TYPE do_update_alias_table(DYN_DNS_CLIENT *p_self)
{
	...
	{			
		for (i = 0; i < p_self->alias_info.count; ++i)
		{
			...
			{
				//fprintf(stderr,"Update DDNS! IP not change!\n");
				continue;
			}	
			...
			/*build dyndns transaction*/
			{
				...
				if (rc == RC_OK)
				{
					...
					//fprintf(stderr,"Update DDNS! start to write ddns_status.log\n");
					...
					if ((fp = fopen("/var/ddns_status.log", "w+")) != NULL ) /* Support DDNS status page, __MSTC__, yic */
					{
						...
						if (update_ok==1)
						{
							...
							else if (strstr(p_self->info.dyndns_server_name.name, NO_IP_SYSTEM_DYNAMIC) != NULL)
							{
								...
								//fprintf(stderr,"Debug:In do_update_alias_table tmp=%s\n",tmp);
								
								if ( !strcmp(tmp,"Thailand") ){
									
									//fprintf(stderr,"Debug:In do_update_alias_table case:Thailand\n"); 
									
									if (strstr(http_tr.p_rsp, "status=1") != NULL)
									{
										fprintf(fp,"/is_success=%d",1); /*updated on server is success.*/
										isServerUpdate = 1;
									}
									else if (strstr(http_tr.p_rsp, "status=0") != NULL)
									{
										fprintf(fp,"/is_success=%d",2); /*updated on server is success but the ip is no chang*/
										isServerUpdate = 1;
									}
...
#if 0  //ZyXEL, Ryan, fix the GUI DDNS status disapear issue		
