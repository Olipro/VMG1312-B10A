...
static RC_TYPE get_dyndns_system_handler(CMD_DATA *p_cmd, int current_nr, void *p_context)
{
	...
	//fprintf(stderr,"Debug:In get_dyndns_system_handler tmp=%s\n",tmp);
	
	if ( !strcmp(tmp,"Thailand") ){
		
		//fprintf(stderr,"Debug:In get_dyndns_system_handler case:Thailand\n"); 
		
		DYNDNS_SYSTEM_INFO_Thailand *it = get_dyndns_system_table_Thailand();
