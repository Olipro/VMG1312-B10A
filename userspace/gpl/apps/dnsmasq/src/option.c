...
//Peter
static void getInterfaceIPByName(char *name, char *ipbuf)
{
	int fd;
	struct ifreq ifr;
	...
	close(fd);
	sprintf(ipbuf, "%s", inet_ntoa(( (struct sockaddr_in *)&ifr.ifr_addr )->sin_addr));
	return;
}
		    ...
		    if ((newlist->source_addr.in.sin_addr.s_addr = inet_addr(source)) == (in_addr_t) -1)
		      {
				...
				getInterfaceIPByName(source, ipbuf);
				if ((newlist->source_addr.in.sin_addr.s_addr = inet_addr(ipbuf)) == (in_addr_t) -1) {
