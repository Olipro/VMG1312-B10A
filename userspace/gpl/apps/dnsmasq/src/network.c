...
struct listener *create_wildcard_listeners(void)
{
  ...
  if (daemon->port != 0)
    {
#ifndef CODETEL_CUSTOMIZATION /*not to create TCP port to support DNS over TCP, chchien for Codetel DNS issue */
	...
	return NULL;
	  ...
	  bind(fd, (struct sockaddr *)&addr, sa_len(&addr)) == -1)
	return NULL;
#else /* not to create TCP port to support DNS over TCP, chchien for Codetel DNS issue */
