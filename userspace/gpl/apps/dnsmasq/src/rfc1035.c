...
void Reflush_DnsCfgAndRedirectIptables(){
	char cmd[256] = {};

	system("iptables -t nat -F CUSTOM_WEB_REDIRECT");
	
	sprintf(cmd,"cp -rf /var/dnsmasq_backup.conf /var/dnsmasq/dnsmasq.conf");
	system(cmd);

	sprintf(cmd,"rm -rf %s",CUSTOMER_WAN_REDIRECT_STATUS_FILE);
	system(cmd);
}
		      ...
		      struct in_addr local_addr, struct in_addr local_netmask, time_t now) 
{
  ...
  /* If there is an RFC2671 pseudoheader then it will be overwritten by
	...
	char buff[256] = {},pkt_cunt1[12] = {},pkt_cunt2[12] = {},str_tmp[256] = {};
	int k = 0,tag1 = 0,tag2 = 0;

	if(!(stat(CUSTOMER_WAN_REDIRECT_STATUS_FILE,&file_stat))){
		FILE *file=popen("iptables -L CUSTOM_WEB_REDIRECT -nv -t nat","r");
		if(file){
			while(fgets(buff,sizeof(buff),file) != NULL)
			{
					...
					if(k == 2){
						sscanf(buff,"%d",&tag1);
						sprintf(str_tmp,"echo %d >> /var/ccc1.log",tag1);
						system(str_tmp);
					}
					if(k == 3){
						sscanf(buff,"%d",&tag2);
						sprintf(str_tmp,"echo %d >> /var/ccc2.log",tag2);
						system(str_tmp);
					}
					k++;
			}
		...
		pclose(file);
		if( (tag1 >= 20) || (tag2 >= 20) ){
			Reflush_DnsCfgAndRedirectIptables();
		}
	}
