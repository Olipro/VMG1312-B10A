...
enum {
	...
	IF_FEATURE_PIDOF_STATUS(	OPTBIT_STATUS  ,)
	IF_FEATURE_PIDOF_OMIT(  OPTBIT_OMIT  ,)
	OPT_SINGLE = IF_FEATURE_PIDOF_SINGLE((1<<OPTBIT_SINGLE)) + 0,
	OPT_STATUS = IF_FEATURE_PIDOF_STATUS(  (1<<OPTBIT_STATUS  )) + 0,
...
/* Jennifer, support ZyIMS watchdog */
#if ENABLE_FEATURE_PIDOF_STATUS	
	int pstatus = 0;
#endif
...
/* Jennifer, support ZyIMS watchdog */
		...
		{
#if ENABLE_FEATURE_PIDOF_STATUS	
			pstatus = find_status_by_name(*argv);
			printf("%d",pstatus);
			break;
#endif
		}
