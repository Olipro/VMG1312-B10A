...
// brcm   FILE *file=NULL;
   ...
   int listcount;
	...
	unsigned short blkSize = TFTP_BLOCKSIZE_DEFAULT;
...
//printf ("mode= %s, file= %s\n", list[1], list[0]);   
   ...
   if (file == NULL)
   {
	...
	for (i=2;i<listcount;i++) {
		if(strcasecmp(list[i],"blksize") == 0){ 
			tmp = atoi(list[++i]);
			if(tmp <= 0) blkSize = TFTP_BLOCKSIZE_DEFAULT;
			else blkSize = tmp;

			if(blkSize < TFTP_BLOCKSIZE_DEFAULT || blkSize > 1428){
				printf("The tftp block size need at %d to 1500 bytes\n", TFTP_BLOCKSIZE_DEFAULT);
				tftpd_nak(peer,EBADOP);
				common_exit(0);
			}
		}
		else if(strcasecmp(list[i],"timeout") == 0){ 
			tmp = atoi(list[++i]);
			if(tmp <= 0) timeout = 3;
			else timeout = tmp;
		}
		else if(strcasecmp(list[i],"tsize") == 0){ 
			tmp = atoi(list[++i]);
			if(tmp <= 0) {tsize = 0;}
			else tsize = tmp;
		}
	...
	buffer = (char*)malloc(sizeof(char)*(blkSize + 4));
	if(buffer == NULL){
		printf("Can not allocate buffer (%d bytes need)\n", blkSize + 4);
		tftpd_nak(peer,EUNDEF);
		common_exit(0);
	}
	pkt=(struct tftphdr *)buffer;
   ...
   do
   {
      ...
      for (i = 0; i < DUP_PKT_CHECK_COUNT; i++)
      {
         ...
         if (block == (int) (*(short*)(buffer+2)))
            break;
#else
		inbytes=tftpd_getdata(peer,block,buffer,blkSize+4);
		//inbytes=tftpd_getdata(peer,block,buffer,blkSize+4);

		if(inbytes < 0){
			printf("Tftp get data error(%d)\n", inbytes);
			tftpd_nak(peer,EUNDEF);
			goto tftpd_receive_fail;
		}
		else if(inbytes == 4 && pkt->th_block == block-1){
			block -= 1;
			break;
		}

		if (block == pkt->th_block)
            break;
#endif 
...
#else
		 if (byteRd < blkSize)   // not enough data for a valid first packet and exit
         {
            cmsLog_error("first packet too short, byteRd=%d default block size=%d", byteRd, blkSize);
			tftpd_nak(peer,EUNDEF);
...
#else
		if (isConfigFile) totalAllocatedSize = cmsImg_getConfigFlashSize();
		else totalAllocatedSize = cmsImg_getImageFlashSizeLimit() + cmsImg_getBroadcomImageTagSize();

		if(tsize != 0){
			if(tsize > totalAllocatedSize){
				printf("The %s file was limit at %d bytes\n",(isConfigFile) ? "config" : "image", totalAllocatedSize);
				tftpd_nak(peer,EUNDEF);
				goto tftpd_receive_fail;
			} else if(tsize < totalAllocatedSize) {
				totalAllocatedSize = tsize;
			}
		}
		if (!isConfigFile) cmsImg_sendLoadStartingMsg(msgHandle, connIfName);
#endif
