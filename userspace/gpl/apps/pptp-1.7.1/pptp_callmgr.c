...
void call_callback(PPTP_CONN *conn, PPTP_CALL *call, enum call_state state)
{
    ...
    switch(state) {
        case CALL_OPEN_DONE:
			...
			printf("call callback call open done\r\n");
            break;
        case CALL_OPEN_FAIL:
			printf("call open fail\r\n");
        case CALL_CLOSE_RQST:
			printf("call open request\r\n");
