...
#ifdef MSTC_ROM_D //__MSTC__, check rom-d uploaded
static char isRomd = 0;
#endif
...
static void processCfgFilter()
{
	   ...
	   char curUserName[]="supervisor";
	   
	   if( imageFormat == CMS_IMAGE_FORMAT_XML_CFG || imageFormat == CMS_IMAGE_FORMAT_XML_CFG_ROM_D){	   
		   if(NULL == (fsin = fopen(inputpath ,"wb+"))){
			   control_printf(SL_FAILURE, "Image updating failed.");
			   result=0;
		   }
		   else if(uploadSize != (count = fwrite(imagePtr, 1, uploadSize, fsin)))
		   {
			   control_printf(SL_FAILURE, "Image updating failed.");
			   result=0;   
		   }
		   ...
		   if(responseMsg->dataLength!=0){
			   ...
			   //if ((imagePtr = cmsMem_alloc(responseMsg->dataLength , ALLOC_ZEROIZE)) == NULL)
			   if ((imagePtr = (char *) malloc(responseMsg->dataLength)) == NULL)
			   {
				   result = 0;
			   }else{	   
...
#endif
#ifdef MSTC_ROM_D
	if (isRomd && (CMS_IMAGE_FORMAT_XML_CFG == imageFormat)) {
...
#if 1 //delete rom-d from ftp
void do_deleteRomd()
{
	char buf[sizeof(CmsMsgHeader) + 1]={0};
	CmsMsgHeader *msg=(CmsMsgHeader *) buf;

	msg->type = CMS_MSG_CLEAN_ROM_D;
	msg->src = EID_TFTP;
	msg->dst = EID_SSK;
	msg->flags_request = 1;
	msg->dataLength = 1;
	...
	{
		control_printf(SL_FAILURE, "451 Error: could not delete ROM-D.");
	}
	else
	{
...
//#if 1 /*ZyXEL Jacky, rename backupsettings.conf to rom-0*/
     ...
     * Send a message to smd to request a copy of the config file.
     */
    requestMsg.src = EID_FTPD;
    requestMsg.dst = EID_SMD;
#ifdef SUPPORT_CONFIGURATION_FILTER
	requestMsg.type = CMS_MSG_GET_CONFIG_FILE_WITH_FILTER;
#else
...
#ifdef MSTC_ROM_D //check filename is /rom-d
	if (!strcmp(platestSlash,"/rom-d"))
#if 1 // __ZyXEL__, Wood, only allow supervisor & admin level account to update rom-d from ftp
		if (userinfo.pw_gid == 0)
		{
			isRomd = 1;
		} else {
...
#if 1 //delete rom-d from ftp
	if (!strcmp(filename, "delete-rom-d")){
		do_deleteRomd();
		free(mapped);
		return;
	}
