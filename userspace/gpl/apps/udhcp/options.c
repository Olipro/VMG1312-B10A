...
struct dhcp_option options[] = {
	...
	{"staticrt33",	OPTION_STATIC_ROUTES_OP33,		0x21},  // __ZyXEL__, Albert, 20131216, for DHCP option 33
	...
	{"serverid",	OPTION_IP,			0x36},       //54
	{"tftp",	OPTION_STRING,			0x42},       //66
	{"bootfile",	OPTION_STRING,			0x43},   //67
	{"sipdomname",	OPTION_STRING,		0x78},	  // __ZyXEL__, Albert, 20131216, for DHCP option 120
#if 1 // __ZyXEL__, Wood, for DHCP option 121 	
	{"staticrt",	OPTION_STATIC_ROUTES,		0x79},
#endif
...
int option_lengths[] = {
	...
	[OPTION_S32] =		4,
#if 1 // __ZyXEL__, Wood, for DHCP option 121
	/* we use minimum length here */	
	[OPTION_STATIC_ROUTES] = 5,
#endif
#if 1 // __ZyXEL__, Albert, 20131216, for DHCP option 33
    [OPTION_STATIC_ROUTES_OP33] = 8,
#endif    
};
...
CmsRet getTFTPServerName(GetTFTPServername *TFTPServerName)
{
   ...
   msg->type = CMS_MSG_GET_TFTP_SERVER_NAME;
   msg->src = EID_DHCPD;
   msg->dst = EID_SSK;
   msg->flags_request = 1;
   ...
   else
   {
      ...
      else
      {
         switch(msg->type)
         {
         case CMS_MSG_GET_TFTP_SERVER_NAME:
            if(msg->dataLength != sizeof(GetTFTPServername))
            {
               cmsLog_error("Invalid Data Length: received %d != expected %d",
                            msg->dataLength, sizeof(GetTFTPServername));

               ret = CMSRET_INTERNAL_ERROR;
               break;
            }
            msgBuf = (GetTFTPServername *)(msg + 1);
            memcpy(TFTPServerName,msgBuf,sizeof(GetTFTPServername));
            break;

         default:
            cmsLog_error("Invalid message type (%x), expects CMS_MSG_GET_TFTP_SERVER_NAME (%x)", (unsigned int)msg->type,CMS_MSG_GET_TFTP_SERVER_NAME);
            ret = CMSRET_INTERNAL_ERROR;
         }
...
CmsRet getDomainName(GetDomainname*InfoDomainname)
{
   ...
   msgBuf = cmsMem_alloc(sizeof(CmsMsgHeader), ALLOC_ZEROIZE);
   msg = (CmsMsgHeader *)msgBuf;
   msg->type = CMS_MSG_GET_Domain_Name;
   msg->src = EID_DHCPD;
   msg->dst = EID_SSK;
   msg->flags_request = 1;
   ...
   else
   {
      ...
      else
      {
         switch(msg->type)
         {
         case CMS_MSG_GET_Domain_Name:
            if(msg->dataLength != sizeof(GetDomainname))
            {
               cmsLog_error("Invalid Data Length: received %d != expected %d",
                            msg->dataLength, sizeof(GetDomainname));

               ret = CMSRET_INTERNAL_ERROR;
               break;
            }
            msgBuf = (GetDomainname *)(msg + 1);
            memcpy(InfoDomainname,msgBuf,sizeof(GetDomainname));
            break;

         default:
            cmsLog_error("Invalid message type (%x), expects CMS_MSG_GET_Domain_Name (%x)", (unsigned int)msg->type,CMS_MSG_GET_Domain_Name);
            ret = CMSRET_INTERNAL_ERROR;
         }
...
CmsRet CheckClientOp125WithConfig(ClientOption125 *Op125)
{
   ...
   msgBuf = cmsMem_alloc(sizeof(CmsMsgHeader), ALLOC_ZEROIZE);
   msg = (CmsMsgHeader *)msgBuf;
   msg->type = CMS_MSG_GET_CHECK_OP125;
   msg->src = EID_DHCPD;
   msg->dst = EID_SSK;
   msg->flags_request = 1;
   ...
   else
   {
      ...
      else
      {
         switch(msg->type)
         {
         case CMS_MSG_GET_CHECK_OP125:
            if(msg->dataLength != sizeof(ClientOption125))
            {
               cmsLog_error("Invalid Data Length: received %d != expected %d",
                            msg->dataLength, sizeof(ClientOption125));

               ret = CMSRET_INTERNAL_ERROR;
               break;
            }
            msgBuf = (ClientOption125 *)(msg + 1);
            memcpy(Op125,msgBuf,sizeof(ClientOption125));
            break;

         default:
            cmsLog_error("Invalid message type (%x), expects CMS_MSG_GET_TFTP_SERVER_NAME (%x)", (unsigned int)msg->type,CMS_MSG_GET_CHECK_OP125);
            ret = CMSRET_INTERNAL_ERROR;
         }
