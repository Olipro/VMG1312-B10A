...
OPENSSL_INC=
OPENSSL_LIB=
AC_DEFUN(OPENSSL_DO_IT_ALL,
[
    dnl Please leave this alone. I use this file in
    dnl oprofile.
    FATAL=1

    AC_ARG_WITH(openssl, [  --with-openssl=PATH      where the root of OpenSSL is installed ],
        [ openssl_cv_dir=`eval echo "$withval"/` ])
     
    AC_ARG_WITH(openssl-includes, [  --with-openssl-includes where the OpenSSL includes are. ],
        [ openssl_cv_includes=`eval echo "$withval"` ])

 
    AC_ARG_WITH(openssl-libraries, [  --with-openssl-libraris where the OpenSSL library is installed.],
        [  openssl_cv_libraries=`eval echo "$withval"` ])

    dnl derive inc/lib if needed
    if test -n "$openssl_cv_dir"; then
        OPENSSL_DIR="$openssl_cv_dir"
        if test -z "$openssl_cv_includes"; then
            openssl_cv_includes=$openssl_cv_dir/include
            if test -f "$openssl_cv_includes/openssl/md5.h"; then
                OPENSSL_INC="-I$openssl_cv_includes"
            fi
        fi
        ...
        if test -n "$openssl_cv_includes"; then
            if test -f "$openssl_cv_includes/openssl/md5.h"; then
                OPENSSL_INC="-I$openssl_cv_includes"
            fi
        fi
        if test -n "$openssl_cv_libraries"; then
            if test -d "$openssl_cv_libraries"; then
                OPENSSL_LIBS="-L$openssl_cv_libraries -lssl -lcrypto"
            fi
        fi
