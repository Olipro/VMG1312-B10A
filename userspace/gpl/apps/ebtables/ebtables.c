...
#if 1 // __MSTC__, Paul Ho, for MBSSID/LAN Isolation, Merged from 406 common trunk
	...
	for (i = 0; i < entries->nentries; i++) {
		...
		if (hlp->in[0] != '\0') {
			...
			if (hlp->invflags & EBT_IIN)
				printf("! ");
			print_iface(hlp->in); // __MSTC__, Paul Ho, for MBSSID/LAN Isolation, Merged from 406 common trunk
		}
		if (hlp->logical_in[0] != '\0') {
			printf("--logical-in ");
			if (hlp->invflags & EBT_ILOGICALIN)
				printf("! ");
			print_iface(hlp->logical_in); // __MSTC__, Paul Ho, for MBSSID/LAN Isolation, Merged from 406 common trunk
		}
		if (hlp->logical_out[0] != '\0') {
			printf("--logical-out ");
			if (hlp->invflags & EBT_ILOGICALOUT)
				printf("! ");
			print_iface(hlp->logical_out); // __MSTC__, Paul Ho, for MBSSID/LAN Isolation, Merged from 406 common trunk
		}
		if (hlp->out[0] != '\0') {
			printf("-o ");
			if (hlp->invflags & EBT_IOUT)
				printf("! ");
			print_iface(hlp->out); // __MSTC__, Paul Ho, for MBSSID/LAN Isolation, Merged from 406 common trunk
		}
