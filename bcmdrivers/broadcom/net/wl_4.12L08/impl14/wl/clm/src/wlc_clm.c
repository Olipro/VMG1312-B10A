 ...
 *
 * $Id: wlc_clm.c 6519 2014-04-18 03:14:33Z $
 */
...
#define FORMAT_VERSION_MAJOR 12 /* BLOB format version major number */
#define FORMAT_VERSION_MINOR 2 /* BLOB format version minor number */
...
typedef struct data_dsc {
	...
	*/
	...
	/** Valid channel comb sets (per band, per bandwidth). Empty for 80+80 */
...
/** Returns registry flags for given data set
 * \param[in] ds_idx Data set index (same as data set ID)
 * \return Registry flags (if present) or 0
 */
static int get_registry_flags(int ds_idx)
{
	const data_dsc_t *ds = get_data_sources(ds_idx);
	/* has_registry_flag is not stored in data_dsc_t (alas!) But
	 * has_region_flags is a good enough proxy for it
	 */
	return ds->has_region_flags ?  ds->data->flags : 0;
}
 ...
 * \param[in] other_in_pair Other channel in 80+80 channel pair. Used when combs parameter is
    zero-length
...
static clm_result_t data_init(const clm_data_header_t *header, data_source_id_t ds_id)
{
	...
	if (header) {
		...
		if (has_registry_flags && (ds->data->flags & CLM_REGISTRY_FLAG_PER_BW_RS)) {
	...
	#ifdef WL11AC
			ds->chan_ranges_bw[CLM_BW_80_80] = ds->chan_ranges_bw[CLM_BW_80] =
	...
	#endif
			ds->rate_sets_bw[CLM_BAND_5G][CLM_BW_20] =
				(const unsigned char *)GET_DATA(ds_id, locale_rate_sets_5g_20m);
			ds->rate_sets_bw[CLM_BAND_5G][CLM_BW_40] =
				(const unsigned char *)GET_DATA(ds_id, locale_rate_sets_5g_40m);
	#ifdef WL11AC
			ds->rate_sets_bw[CLM_BAND_5G][CLM_BW_80_80] =
				ds->rate_sets_bw[CLM_BAND_5G][CLM_BW_80] =
				(const unsigned char *)GET_DATA(ds_id, locale_rate_sets_5g_80m);
			ds->rate_sets_bw[CLM_BAND_5G][CLM_BW_160] =
				(const unsigned char *)GET_DATA(ds_id, locale_rate_sets_5g_160m);
	#endif
			if (has_registry_flags &&
				(ds->data->flags & CLM_REGISTRY_FLAG_PER_BAND_RATES)) {
				ds->rate_sets_bw[CLM_BAND_2G][CLM_BW_20] =
					(const unsigned char *)GET_DATA(ds_id,
					locale_rate_sets_2g_20m);
				ds->rate_sets_bw[CLM_BAND_2G][CLM_BW_40] =
					(const unsigned char *)GET_DATA(ds_id,
			...
			for (bw_idx = 0; bw_idx < CLM_BW_NUM; ++bw_idx) {
				...
				for (band_idx = 0; band_idx < CLM_BAND_NUM; ++band_idx) {
					ds->rate_sets_bw[band_idx][bw_idx] = rate_sets;
				}
			}
		}
	} else {
...
static clm_result_t data_init_shim(const clm_data_header_t *header, data_source_id_t ds_id)
{
	clm_result_t ret = data_init(header, ds_id);
	int registry_flags;
	if (ret != CLM_RESULT_OK) {
		return ret;
	}
	registry_flags = get_registry_flags(ds_id);
	if (registry_flags & ~CLM_REGISTRY_FLAG_ALL) {
		return CLM_RESULT_ERR;
	}
	if (registry_flags & CLM_REGISTRY_FLAG_REGION_LOC_12_FLAG_SWAP) {
		get_data_sources(ds_id)->country_rev_rec_len++;
	}
	return ret;
}
#define data_init data_init_shim
	...
	const clm_country_rev_definition10_fl_t *country_definition, int loc_type)
{
	...
	if (get_data_sources(ds_id)->loc_idx10) {
		...
		if (get_registry_flags(ds_id) & CLM_REGISTRY_FLAG_REGION_LOC_12_FLAG_SWAP) {
			ret |= ((int)(((const clm_country_rev_definition_cd10_t *)
				country_definition)->extra[2]) <<
	...
	for (base_ht_idx = 0; base_ht_idx < (int)ARRAYSIZE(base_ht_loc_data); ++base_ht_idx) {
		...
		do {
			...
			if ((flags & CLM_DATA_FLAG_WIDTH_MASK) == CLM_DATA_FLAG_WIDTH_40) {
				...
				((flags & CLM_DATA_FLAG_WIDTH_MASK) == CLM_DATA_FLAG_WIDTH_80_80) {
	...
	const clm_limits_params_t *params, clm_power_limits_t *limits)
{
	...
	for (base_ht_idx = 0; base_ht_idx < (int)ARRAYSIZE(base_ht_loc_data); ++base_ht_idx) {
		...
		if (limits_type == CLM_LIMITS_TYPE_CHANNEL) {
			...
			fill_actual_subchan_table(bw_to_chan, subchannels, limits_type, channel,
				country_data.chan_ranges_bw[CLM_BW_80], comb_sets[CLM_BW_80],
				country_data.sub_chan_channel_rules_80.channel_rules,
				country_data.sub_chan_channel_rules_80.num,
				sizeof(clm_sub_chan_channel_rules_80_t));
		...
		do {
			...
			/* Channel combs for bandwidth used in subsequence. NULL for 80+80 chan */
			...
			for (num_rec = (int)*tx_rec++; num_rec--; tx_rec += tx_rec_len)
			{
				...
				if (tx_power_idx >= tx_rec_len) {
					...
					if (!valid_channel && channel_for_bw &&
						...
						range, comb_set_for_bw, params->other_80_80_chan) &&
...
const char* clm_get_string(clm_string_type_t string_type, clm_string_source_t string_source)
{
	data_source_id_t ds_id =
		((string_source == CLM_STRING_SOURCE_BASE) ||
		((string_source == CLM_STRING_SOURCE_EFFECTIVE) && !get_data_sources(DS_INC)->data))
			? DS_MAIN : DS_INC;
	const data_dsc_t *ds = get_data_sources(ds_id);
	const clm_data_header_t *header = ds->header;
	const char* ret = NULL;
...
static clm_result_t
remap_regrev(const ccode_t cc, unsigned *rev, MY_BOOL map_16_to_8,
	clm_result_t result)
{
	const clm_regrev_cc_remap_set_t *regrev_remap_set = NULL;
	int ds_idx;
	unsigned int num_regrevs;
	const clm_regrev_cc_remap_t *cc_remap, *cc_remap_end;
	const clm_regrev_regrev_remap_t *regrev_remap;
	if (result != CLM_RESULT_OK) {
		return result;
	}
	for (ds_idx = 0; ds_idx < DS_NUM; ds_idx++) {
		const data_dsc_t *ds = get_data_sources(ds_idx);
		if (ds->has_region_flags &&
			(ds->data->flags & CLM_REGISTRY_FLAG_REGREV_REMAP) &&
			((regrev_remap_set = (const clm_regrev_cc_remap_set_t *)
			GET_DATA(ds_idx, regrev_remap)) != NULL))
		{
			break;
		}
	}
	if (!regrev_remap_set || !regrev_remap_set->num) {
		return result;
	}
	cc_remap = (const clm_regrev_cc_remap_t *)relocate_ptr(ds_idx,
		regrev_remap_set->cc_remaps);
	for (cc_remap_end = cc_remap + regrev_remap_set->num;
			cc_remap < cc_remap_end; ++cc_remap)
	{
		if (cc_equal(cc, cc_remap->cc)) {
			break;
		}
	}
	...
	regrev_remap = (const clm_regrev_regrev_remap_t *)relocate_ptr(ds_idx,
		regrev_remap_set->regrev_remaps) + cc_remap->index;
	for (num_regrevs = cc_remap[1].index - cc_remap->index; num_regrevs--;
			++regrev_remap)
	{
		unsigned int r16 = (unsigned int)regrev_remap->r16l +
			((unsigned int)regrev_remap->r16h << 8);
		if (*rev == (map_16_to_8 ? r16 : regrev_remap->r8)) {
			*rev = map_16_to_8 ? regrev_remap->r8 : r16;
			return CLM_RESULT_OK;
		}
	}
	return CLM_RESULT_ERR;
...
extern clm_result_t
clm_country_iter_shim(clm_country_t *country, ccode_t cc, unsigned int *rev);
clm_result_t
clm_country_iter_shim(clm_country_t *country, ccode_t cc, unsigned int *rev)
{
	clm_result_t ret = clm_country_iter(country, cc, rev);
	return remap_regrev(cc, rev, FALSE, ret);
}

extern clm_result_t
clm_country_lookup_shim(const ccode_t cc, unsigned int rev,
	clm_country_t *country);
clm_result_t
clm_country_lookup_shim(const ccode_t cc, unsigned int rev,
	clm_country_t *country)
{
	remap_regrev(cc, &rev, TRUE, CLM_RESULT_OK);
	return clm_country_lookup(cc, rev, country);
}

extern clm_result_t
clm_agg_country_iter_shim(clm_agg_country_t *agg, ccode_t cc,
	unsigned int *rev);
clm_result_t
clm_agg_country_iter_shim(clm_agg_country_t *agg, ccode_t cc,
	unsigned int *rev)
{
	clm_result_t ret = clm_agg_country_iter(agg, cc, rev);
	return remap_regrev(cc, rev, FALSE, ret);
}

extern clm_result_t
clm_agg_map_iter_shim(const clm_agg_country_t agg, clm_agg_map_t *map,
	ccode_t cc, unsigned int *rev);
clm_result_t
clm_agg_map_iter_shim(const clm_agg_country_t agg, clm_agg_map_t *map,
	ccode_t cc, unsigned int *rev)
{
	clm_result_t ret = clm_agg_map_iter(agg, map, cc, rev);
	return remap_regrev(cc, rev, FALSE, ret);
}

extern clm_result_t
clm_agg_country_lookup_shim(const ccode_t cc, unsigned int rev,
	clm_agg_country_t *agg);
clm_result_t
clm_agg_country_lookup_shim(const ccode_t cc, unsigned int rev,
	clm_agg_country_t *agg)
...
extern clm_result_t
clm_agg_country_map_lookup_shim(const clm_agg_country_t agg,
	const ccode_t target_cc, unsigned int *rev);
clm_result_t
clm_agg_country_map_lookup_shim(const clm_agg_country_t agg,
	const ccode_t target_cc, unsigned int *rev)
{
	clm_result_t ret = clm_agg_country_map_lookup(agg, target_cc, rev);
	return remap_regrev(target_cc, rev, FALSE, ret);
}
