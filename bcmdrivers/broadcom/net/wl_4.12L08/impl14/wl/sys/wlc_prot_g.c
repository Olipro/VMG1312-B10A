...
static int wlc_prot_g_watchdog(void *ctx);
#if defined(BCMDBG)
...
BCMATTACHFN(wlc_prot_g_attach)(wlc_info_t *wlc)
{
	...
	if ((prot = wlc_calloc(wlc->osh, wlc->pub->unit, WLC_PROT_G_SIZE)) == NULL) {
...
wlc_prot_g_bss_init(void *ctx, wlc_bsscfg_t *cfg)
{
	...
	if ((pg = wlc_calloc(wlc->osh, wlc->pub->unit, BSS_PROT_G_SIZE)) == NULL) {
		WL_ERROR(("wl%d: %s: out of mem, malloced %d bytes\n",
			wlc->pub->unit, __FUNCTION__, MALLOCED(wlc->osh)));
		err = BCME_NOMEM;
		goto fail;
...
#ifndef MACOSX     /* Only for non MAC platforms */
...
_wlc_prot_g_ovlp_upd(wlc_prot_g_info_t *prot, wlc_bsscfg_t *cfg,
	chanspec_t chspec, uint8 *erp, int erp_len, bool is_erp, bool short_cap)
{
	bss_prot_g_cfg_t *pgc;
	bss_prot_g_to_t *pgt;
	...
	uint8 *erp, int erp_len, bool is_erp, bool short_cap)
{
	...
	FOREACH_AS_BSS(wlc, i, bc) {
		...
		_wlc_prot_g_ovlp_upd(prot, bc, chspec, erp, erp_len, is_erp, short_cap);
	}
}
...
uint8 *
wlc_prot_g_write_erp_ie(wlc_prot_g_info_t *prot, wlc_bsscfg_t *cfg, uint8 *buf, int buflen)
{
	...
	buf = wlc_write_info_elt_safe(buf, buflen, DOT11_MNG_ERP_ID, DOT11_MNG_ERP_LEN, erp_flags);

	/* Legacy NonERP Information Element if required */
	if (pgc->include_legacy_erp) {
		buf = wlc_write_info_elt_safe(buf, buflen, DOT11_MNG_NONERP_ID, 1, erp_flags);
	}
