...
static int itfr_stats_watchdog(void *ctx);
...
BCMATTACHFN(wlc_itfr_attach)(wlc_info_t *wlc)
{
	...
	if ((itfr = wlc_calloc(wlc->osh, wlc->pub->unit, sizeof(itfr_info_t))) == NULL) {
	...
	if (wlc_module_register(wlc->pub, wlc_itfr_iovars, ITFR_MODULE_NAME,
...
BCMATTACHFN(wlc_itfr_detach)(itfr_info_t *itfr)
{
	...
	wlc_module_unregister(wlc->pub, ITFR_MODULE_NAME, itfr);
	MFREE(wlc->osh, itfr, sizeof(itfr_info_t));
}
...
static void
itfr_init_stats(itfr_info_t *itfr)
{
	/* init variables */
