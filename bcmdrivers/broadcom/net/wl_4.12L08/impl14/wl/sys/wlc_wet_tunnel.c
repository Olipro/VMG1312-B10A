 ...
 *
 * $Id: wlc_wet_tunnel.c 344557 2012-07-13 03:21:12Z $
 */
#include <wlc_cfg.h>
#include <typedefs.h>
#include <bcmdefs.h>
...
static int wet_tunnel_stas_timeout(void *cntxt);
static int wet_tunnel_sta_free(wlc_wet_tunnel_info_t *weth, wet_tunnel_sta_t *sta);
static void wet_tunnel_sta_clean_all(wlc_wet_tunnel_info_t *weth);
static int wet_tunnel_sta_remove_ip_entry(wlc_wet_tunnel_info_t *weth, uint8 *iaddr);
...
BCMATTACHFN(wlc_wet_tunnel_attach)(wlc_info_t *wlc)
{
	...
	wlc_module_register(wlc->pub, wet_tunnel_iovars, "wet_tunnel", weth,
		wet_tunnel_doiovar, wet_tunnel_stas_timeout, NULL, NULL);
#ifdef BCMDBG
...
wet_tunnel_eth_proc(wlc_wet_tunnel_info_t *weth, void *sdu, int send)
{
	...
	int length = pkttotlen(WLCOSH(weth), sdu);
...
/* free IP/MAC mapping entry */
/* Returns 0 if succeeded; < 0 otherwise. */
static int
wet_tunnel_sta_free(wlc_wet_tunnel_info_t *weth, wet_tunnel_sta_t *sta)
{
	wet_tunnel_sta_remove_ip_entry(weth, sta->ip);
	wet_tunnel_sta_remove_mac_entry(weth, &sta->mac);
	bzero(sta, sizeof(wet_tunnel_sta_t));

	sta->next = weth->stafree;
	weth->stafree = sta;
...
/*  Remove IP entry from hash list */
static int
wet_tunnel_sta_remove_ip_entry(wlc_wet_tunnel_info_t *weth, uint8 *iaddr)
{
	wet_tunnel_sta_t *sta, *prev;
	...
	/* find the existing one */
	for (sta = prev = weth->stahash_ip[i]; sta; sta = sta->next_ip) {
		if (!bcmp(sta->ip, iaddr, IPV4_ADDR_LEN)) {
			found = 1;
			break;
		}
	...
	/* bail out if we can't find */
	if (!found) {
		WL_ERROR(("wl%d: wet_tunnel_sta_remove_ip_entry: unable to find STA "
			"%u.%u.%u.%u entry\n",
			WLCUNIT(weth), iaddr[0], iaddr[1], iaddr[2], iaddr[3]));
		return -1;
	}
	...
	/* fix the list */
	if (prev == sta)
		weth->stahash_ip[i] = sta->next_ip; /* removing first entry in this bucket */
	else
		prev->next_ip = sta->next_ip;
...
/* Inactivity timeout for WET STA */
static int
wet_tunnel_stas_timeout(void *cntxt)
{
	wlc_wet_tunnel_info_t *weth = (wlc_wet_tunnel_info_t *)cntxt;
	int i;
	wet_tunnel_sta_t *sta;
#ifdef BCMDBG
	char eabuf[ETHER_ADDR_STR_LEN];
	...
	/* ignore if not in AP mode */
	if (!AP_ENAB(weth->pub))
		return BCME_OK;

	/* don't do timeout for now, because we don't recover from it */
	return BCME_OK;

	/* find the existing one by MAC */
	for (i = 0; i < WET_TUNNEL_STA_HASH_SIZE; i++) {
		for (sta = weth->stahash_mac[i]; sta; sta = sta->next_mac) {
			/* idle for long time free it now */
			if ((weth->pub->now - sta->used) >= weth->sta_timeout) {
#ifdef BCMDBG
				printf("wet_tunnel_stas_timeout: timeout %s (Idle %d)\n",
					bcm_ether_ntoa(&sta->mac, eabuf),
					(weth->pub->now - sta->used));
#endif /* BCMDBG */

				wet_tunnel_sta_free(weth, sta);
			}
		}
	}
	return BCME_OK;
	...
	struct scb *scb, struct wlc_if *wlcif, void *sdu)
{
	...
	if ((tmp = PKTGET(osh, TXOFF + totlen, TRUE)) != NULL) {
		...
		wlc_pkttag_info_move(wlc->pub, sdu, tmp);
