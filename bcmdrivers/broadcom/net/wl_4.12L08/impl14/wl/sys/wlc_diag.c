 ...
 *
 * $Id: wlc_diag.c 340256 2012-06-21 19:20:50Z $
 */
...
wlc_diag_lb_int_ext(wlc_info_t *wlc, uint32 lb_int)
{
	...
	while (txcnt < MAX_COUNT || rxcnt < MAX_COUNT) {
		...
		while (txcnt < MAX_COUNT) {
			...
			dma_txfast(wlc_hw->di[TX_DATA_FIFO], p, TRUE);
		...
		while (rxcnt < MAX_COUNT) {
			...
			rx_bi[rxcnt].len = PKTLEN(wlc->osh, rx_p);
	...
	for (i = 0; i < MAX_COUNT; i ++) {
		...
		rx_data = (uint8 *)PKTDATA(wlc->osh, rx_bi[i].pkt) + rx_bi[i].off;
	...
	for (i = 0; i < MAX_COUNT; i ++) {
		PKTFREE(wlc->osh, tx_bi[i].pkt, FALSE);
		PKTFREE(wlc->osh, rx_bi[i].pkt, FALSE);
	}
