 ...
 *
 * $Id: wlc_high_stubs.c 363496 2012-10-18 01:15:05Z $
 */
...
wlc_rpc_bmac_dispatch(wlc_rpc_ctx_t *rpc_ctx, struct rpc_buf* buf)
{
	...
	case WLRPC_WLC_PHY_ISMUTED_ID: {
		...
		muted = wlc_phy_ismuted(((wlc_hw_info_t *)wlc_hw)->band->pi);
	...
	case WLRPC_WLC_PHY_TXPOWER_SROMLIMIT_ID: {
		...
		err = bcm_xdr_unpack_uint32(&b, &temp);
		ASSERT(!err);
		...
		rpc_buf = wlc_rpc_buf_alloc(rpc, &retb, len, WLRPC_WLC_PHY_TXPOWER_SROMLIMIT_ID);
		...
		if (max_pwr) {
			err = bcm_xdr_pack_opaque_varlen(&retb, slen, tbuf);
			ASSERT(!err);
		}
	...
	case WLRPC_WLC_BMAC_WRITE_AMT_ID: {
		...
		wlc_bmac_write_amt(wlc_hw, idx, &addr, attr);
		break;
	}
...
#define HANDLE_BOUNDARY(b, p, prev_p)						\
	...
	case WLRPC_WLC_PHY_TXPOWER_GET_CURRENT_ID: {
		...
		rpc_buf = wlc_rpc_buf_alloc(rpc, &retb, size, WLRPC_WLC_PHY_TXPOWER_GET_CURRENT_ID);
		ASSERT(rpc_buf != NULL);
		...
		err = bcm_xdr_pack_opaque(&retb, size, ((uint8 *)power) + offset);
		ASSERT(!err);
		...
		rpc_buf = wlc_rpc_buf_alloc(rpc, &retb, size + sizeof(uint32),
	...
	case WLRPC_WLC_PHY_BAND_FIRST_CHANSPEC_ID: {
		...
		pi = ((wlc_hw_info_t *)wlc_hw)->bandstate[band_index]->pi;
	...
	case WLRPC_WLC_PHY_INTERF_RSSI_UPDATE: {
		...
		err = bcm_xdr_unpack_int32(&b, &chanspec);
		ASSERT(!err);
...
wlc_update_txppr_offset(wlc_info_t *wlc, int8 *txpwr)
{
	...
	len = ROUNDUP(sizeof(txppr_t), 4);

	rpc_buf = wlc_rpc_buf_alloc(rpc, &b, len, WLRPC_WLC_BMAC_TXPPR_ID);
	ASSERT(rpc_buf != NULL);

	err = bcm_xdr_pack_opaque(&b, sizeof(txppr_t), txpwr);
	ASSERT(!err);
