 ...
 *
 * $Id: wlc_wet.c 355047 2012-09-05 07:00:54Z $
 */
...
#ifdef PLC_WET
static int wet_gigle_proc(wlc_wet_info_t *weth, uint8 *eh, uint8 *gh, int length, int send);
#endif	/* PLC_WET */
...
#ifdef PLC_WET
#define WLCBSSID(weth)  ((weth)->BSSID.octet)
#endif /* PLC_WET */
...
#ifdef PLC_WET
	if (PLC_ENAB(weth->pub)) {
		if (type == HTON16(ETHER_TYPE_88E1))
			return wet_gigle_proc(weth, eh, eh + ETHERVLAN_HDR_LEN,
			                      length, send);
	}
#endif	/* PLC_WET */
...
#ifdef PLC_WET
	/* ARP request or reply frame received, add src address as wifi node */
	if (PLC_ENAB(weth->pub) && !send)
		wlc_plc_wet_node_add(weth->wlc, NODE_TYPE_WIFI_PLC,
		                 eh + ETHER_SRC_OFFSET, WLCBSSID(weth));
#endif /* PLC_WET */
...
#ifdef PLC_WET
static int
wet_gigle_proc(wlc_wet_info_t *weth, uint8 *eh, uint8 *gh, int length, int send)
{
	/* Modify/Restore source MAC in Ethernet header */
	if (send)
		bcopy(WLCHWADDR(weth), eh + ETHER_SRC_OFFSET, ETHER_ADDR_LEN);
	else
		bcopy(gh + length - 6, eh + ETHER_SRC_OFFSET, ETHER_ADDR_LEN);

#if defined(BCMDBG) && defined(PLCDBG)
	prhex("->WIRELESS (modify/restore gigle mac)", eh, length+ETHERVLAN_HDR_LEN);
#endif /* BCMDBG && PLCDBG */
...
void
wlc_wet_bssid_upd(wlc_wet_info_t *weth, wlc_bsscfg_t *cfg)
{
	if (ETHER_ISNULLADDR(cfg->BSSID.octet))
		return;

	bcopy(&cfg->BSSID, &weth->BSSID, ETHER_ADDR_LEN);
}
#endif	/* PLC_WET */
	...
	uint8 *eh, uint8 *iph, uint8 *udph, int length, int send)
{
	...
	for (udphdlr = &udp_hash[WET_UDP_HASH((uint8 *)&port)];
		...
		if (udphdlr->port != port || !udphdlr->udp_proc)
			continue;
#ifdef PLC_WET
		/* DHCP request or reply frame received, use src mac to add
		 * wifi node
		 */
		if (PLC_ENAB(weth->pub) && !send)
			wlc_plc_wet_node_add(weth->wlc, NODE_TYPE_WIFI_PLC,
	...
	uint8 *eh, uint8 *iph, uint8 *udph, uint8 *dhcp, int length, int send)
{
	...
	iaddr = dhcp + DHCP_YIADDR_OFFSET;
	if (wet_sta_update_all(weth, iaddr,
	(struct ether_addr*)(dhcp + DHCP_CHADDR_OFFSET), &sta) < 0) {
		WL_ERROR(("wl%d: wet_dhcp_proc: unable to update STA %u.%u.%u.%u %s\n",
			WLCUNIT(weth), iaddr[0], iaddr[1], iaddr[2], iaddr[3],
			bcm_ether_ntoa(
				(struct ether_addr*)(eh + ETHER_SRC_OFFSET), eabuf)));
	}
...
#ifdef PLC_WET
	if (PLC_ENAB(weth->pub) && ETHER_ISNULLADDR(WLCBSSID(weth)))
		return 0;
#endif /* PLC_WET */
