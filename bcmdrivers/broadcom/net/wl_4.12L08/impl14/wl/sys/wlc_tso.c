 ...
 *
 * $Id: wlc_tso.c 312873 2012-02-04 02:51:39Z $
 */
...
wlc_tso_parse_d11(wlc_info_t *wlc, d11ac_tso_t* tso, void* pt, void* p, struct scb *scb,
	wsec_key_t *key, uint nfrags, uint offset)
{
	struct dot11_header * h = (struct dot11_header *) pt;
	uint len = 0, tot_len = 0;
	uint16 fc, type;
	wlc_pkttag_t *pkttag;
	bool qos;
	osl_t *osh = wlc->osh;
...
#ifdef DEBUG_TSO
	prhex("d11 hdr dump:", (uint8 *)h, len);
#endif
...
#define OFFSET_INCR(a) \
	...
	if ((delta = wlc_tso_parse_d11(wlc, tso, h, p, scb, key, nfrags, offset)) < 0) {
		passthrough = TRUE;
		goto tso_exit;
	}
	OFFSET_INCR(delta);
 ...
 */
void
wlc_toe_add_hdr(wlc_info_t *wlc, void *p, struct scb *scb, wsec_key_t *key,
	uint nfrags, uint16 *pushlen)
{
	...
	wlc_tso_ext_info(wlc, &tso, p, scb, key, nfrags);
