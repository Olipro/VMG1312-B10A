...
#ifdef MSTC_WPS_SPECIAL_CHAR_VERIFY // __MSTC__, Paul Ho, WPS Special Character Verification on SSID/PSK
/*extern*/ int rutWlan_verifyPresharedKEY(char* presharedKey);
/*extern*/ int rutWlan_verifySSIDname(char* ssidname);
#endif /* MSTC_WPS_SPECIAL_CHAR_VERIFY */
...
#ifdef SUPPORT_ENR_BLOCK // __MSTC__, Paul Ho WEP block  in ap-pbc / Open block in ap-pbc / check SSID name
         if(e->reg_info->enrollee->b_ap){
            if(encrSettings != NULL){
               int valid=reg_proto_testApEnrSetting(encrSettings);
         
               if(valid == 0){
                  TUTRACE((TUTRACE_INFO, "%s@%d Reject client set ap-pin by WEP, only WPA-PSK/WPA2-PSK allowed.\n", __FUNCTION__,__LINE__));	
                  configError = WPS_ERROR_SETUP_LOCKED;
                  retVal = state_machine_send_nack(e->reg_info, configError, outmsg, regNonce);
                  enr_sm_restartSM(e);
            ...
            //not define for CTlvEsM8STA
         }
#endif /* SUPPORT_ENR_BLOCK */
...
out:
#ifdef MSTC_WLAN_WPS_FOR_TR69 // __MSTC__, Paul Ho TR069 for wps
   //if error happen, record the configError code to /var/wirelessstatus  	
   if (WPS_SUCCESS != err){
      wps_setFileVarInt("wpsLastConfigErr", configError);
      //wps_sendUpdateConfErrCmsMsg();  //move to wps_ap
   }else{
      wps_setFileVarInt("wpsLastConfigErr", WPS_CONFIG_ERROR_NO_ERROR);
      //wps_sendUpdateConfErrCmsMsg();  //move to wps_ap
   }
#endif /* MSTC_WLAN_WPS_FOR_TR69 */
	return retVal;
...
#ifdef SUPPORT_ENR_BLOCK //__MSTC__, Paul Ho, WEP block  in ap-pbc / Open block in ap-pbc / check SSID name
int reg_proto_testApEnrSetting(void *encrSettings){
   //Reference to wps_get_credentials() wps_enr_core.c
   //block share and open authType

   int valid = 0;

   EsM8Ap *p_tlvEncr = (EsM8Ap *)(encrSettings);
   ...
   uint16 data16;
	...
	CTlvNwKey *p_tlvKey;
	char *cp_data, *nwKey;
	WPS_SSLIST *itr =p_tlvEncr->nwKey;

	cp_data = (char *)(p_tlvEncr->ssid.m_data);
	TUTRACE((TUTRACE_INFO, "reg_proto_testApEnrSetting: Ssid [%s]\n", cp_data));	
	
	p_tlvKey = (CTlvNwKey *)itr;
	//TUTRACE((TUTRACE_INFO, "reg_proto_testApEnrSetting: tlv key : %p \n",p_tlvKey));
	nwKey = (char *)p_tlvKey->m_data;
	TUTRACE((TUTRACE_INFO, "reg_proto_testApEnrSetting: PSK info [%s] \n",nwKey)); 
 
	if(rutWlan_verifySSIDname(cp_data)!=0){
		valid=0;		
		TUTRACE((TUTRACE_INFO, "reg_proto_testApEnrSetting: %d, ssid name is invalid.\n", __LINE__));	
		return valid;
	}
#endif /* MSTC_WPS_SPECIAL_CHAR_VERIFY */
   ...
   else  if (mgmt_data == 0x0002) {
      //WPA-PSK
      valid = 1;
#ifdef MSTC_WPS_SPECIAL_CHAR_VERIFY // __MSTC__, Paul Ho, WPS Special Character Verification on SSID/PSK
	  if(rutWlan_verifyPresharedKEY(nwKey)!=0){
	  	valid=0;
		TUTRACE((TUTRACE_INFO, "reg_proto_testApEnrSetting: %d, PreSharedKey is invalid.\n", __LINE__));	
	  }
#endif /* MSTC_WPS_SPECIAL_CHAR_VERIFY */
   }
   else if (mgmt_data == 0x0020) {
      //WPA2-PSK
      valid = 1;
#ifdef MSTC_WPS_SPECIAL_CHAR_VERIFY // __MSTC__, Paul Ho, WPS Special Character Verification on SSID/PSK
	  if(rutWlan_verifyPresharedKEY(nwKey)!=0){
   ...
   else if (mgmt_data == (0x0002 | 0x0020)) {
      //WPA-PSK WPA2-PSK
      valid = 1;
#ifdef MSTC_WPS_SPECIAL_CHAR_VERIFY // __MSTC__, Paul Ho, WPS Special Character Verification on SSID/PSK
	  if(rutWlan_verifyPresharedKEY(nwKey)!=0){
...
#ifdef MSTC_WPS_SPECIAL_CHAR_VERIFY // __MSTC__, Paul Ho, WPS Special Character Verification on SSID/PSK
// Functions defined here have to sync to rut_wlan.c
/* This function is used by TR69(in RCL)/TR64(in RCL)/WPS
  * return value:   1: invalid character;       0:valid character;   */
int rutWlan_isRejectSpecialChar(char c){
	int i=0;
	char rejectSpecialChar[15]={'/',' ',';','[',']','<','>','&','\'','"','\0'};//	/ ;[]<>&'" (){}:?,.
	for(i=0;i<strlen(rejectSpecialChar);i++){		
		if(c==rejectSpecialChar[i])
			return 1;	
	}
	return 0;
}
...
/* This function is used by TR69(in RCL)/TR64(in RCL)/WPS
  * return value:   0: valid ssid;       !0:invalid ssid;   */
int rutWlan_verifySSIDname(char* ssidname){
	if(!ssidname)
		return 1;

	//cmsLog_debug("verify ssidname:%s",ssidname);	
	int i=0,j=0;
...
/* This function is used by TR69(in RCL)/TR64(in RCL)/WPS
  * return value:   0: valid presharedKey;       !0:invalid presharedKey;   */
int rutWlan_verifyPresharedKEY(char* presharedKey){
	if(!presharedKey)
		return 1;
	
	//cmsLog_debug("verify PreSharedKey:%s",presharedKey);	
	int i=0,j=0;	
	int keyLength = strlen(presharedKey);
	if(keyLength<8) 
		return 1;

	bool isAcceptSpecialChar=FALSE;
	char acceptSpecialChar[10]={'-','_','.','\0'};//for example	
	for(i=0;i<keyLength;i++){		
		if( !( (presharedKey[i]>='A'&&presharedKey[i]<='Z') || 
			   (presharedKey[i]>='a'&&presharedKey[i]<='z') || 
			   (presharedKey[i]>='0'&&presharedKey[i]<='9')))
		{
			if(keyLength!=64){//ASCII
				isAcceptSpecialChar=FALSE;
				for(j=0;j<strlen(acceptSpecialChar);j++){
					 if(presharedKey[i]==acceptSpecialChar[j]){
					 	isAcceptSpecialChar=TRUE;
						break;
					 }
				}
				if(!isAcceptSpecialChar){
					//cmsLog_error("Preshared Key is invalid. ErrorChar=%c \n",presharedKey[i]);			
					return 1;	
				}
			}
		}
