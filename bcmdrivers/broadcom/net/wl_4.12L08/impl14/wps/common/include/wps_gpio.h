...
#if defined(MSTC_WPS_BTN_SCE) // __MSTC__, Paul Ho, for WPS button
#if defined(MTS_WPS_BTN_PER)
	#define WPS_LONG_PRESSTIME	MTS_WPS_BTN_PER			/* seconds */
#else
	#define WPS_LONG_PRESSTIME	5			/* seconds */
#endif
	#define WPS_BTNSAMPLE_PERIOD	(100 * 1000)	/* 100 ms Sync with board.c sample rate*/ 
#else /* defined(MSTC_WPS_BTN_SCE) */
#define WPS_LONG_PRESSTIME	5			/* seconds */
#define WPS_BTNSAMPLE_PERIOD	(500 * 1000)	/* 500 ms */
#endif /* defined(MSTC_WPS_BTN_SCE) */ // end, __MSTC__, Paul Ho, for WPS button
