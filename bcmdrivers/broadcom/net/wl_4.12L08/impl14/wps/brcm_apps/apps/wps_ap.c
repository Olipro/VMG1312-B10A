...
#if 1 // __MSTC__, Paul Ho, __BELL__, SinJia, No change ssid after unconfig ap-pin      
      //pass the gen ssid function
#else
...
#ifdef MSTC_WLAN_WPS_FOR_TR69 // __MSTC__, Paul Ho, __Qwest__, SinJia, TR069 for wps
...
#if 1 // __MSTC__, Paul Ho, __Qwest__, Ling-Chun
	if((wps_initCmsMsg()) == 0){
...
#if 1 // __MSTC__, Paul Ho, __OBM__, SinJia, WPS Current status
	if(retVal == WPS_SUCCESS){
		wps_setFileVar("wpsGuiMsg", "result@Success");
	}else if(retVal == WPS_MESSAGE_PROCESSING_ERROR){
		wps_setFileVar("wpsGuiMsg", "result@Unkonwn error");
	}else if(retVal == WPS_ERR_REGISTRATION_PINFAIL){
		wps_setFileVar("wpsGuiMsg", "result@Client pin error");
		retVal = WPS_MESSAGE_PROCESSING_ERROR;
	}
#endif
