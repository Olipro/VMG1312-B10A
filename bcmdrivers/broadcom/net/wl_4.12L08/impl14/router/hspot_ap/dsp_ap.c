...
#ifdef DSLCPE
	// No need for convert as we are using big endian
#else
...
#endif
	TRACE(TRACE_VERBOSE,"wlevent->datalen:%d,length is %d and bcm_event_t length is :%d\n",wlEvent->datalen,length,sizeof(bcm_event_t));
...
static void *dspMonitor(void *arg)
{
	...
	while (1) {
		...
		if (ret == -1) {
#ifdef DSLCPE
			TRACE(TRACE_DEBUG, "select failed %d\n", ret);
#else
