...
#ifdef PARUL_PREMIUM_LOGO_CHANGE
#include "bcmnvram.h"
//#include "wlutils.h"
#include "wlioctl.h"
#include "shutils.h"
...
extern char * lltd_make_wl_prefix(char *prefix,int prefix_size, int mode, char *ifname);


static int premium_qos_get_link_speed();
#endif
...
#ifdef PARUL_CHANGE
	printf("stamp_time: entered - Parul \n");
#endif
...
get_raw_samples(void)
{
  ...
  IF_TRACED(TRC_TLVINFO)
	printf("get_raw_samples: entered - Parul \n");
  END_TRACE
#endif
...
#ifdef PARUL_PREMIUM_LOGO_CHANGE
	g_reorder_buffer = 0;
	g_reorder_buffer = premium_qos_get_link_speed();
	IF_TRACED(TRC_TLVINFO)	
		printf("premium_qos_get_link_speed returned = %d\n", g_reorder_buffer);
		printf("premium_qos_get_link_speed returned htonl'ed = %d\n", htonl(g_reorder_buffer));
	END_TRACE
		
	g_reorder_buffer = 	htonl(g_reorder_buffer);
...
#ifdef PARUL_CHANGE
	IF_TRACED(TRC_QOS)
		printf("qos_probe: entered - Parul \n");
	END_TRACE
#endif
...
#ifdef PARUL_CHANGE
	IF_TRACED(TRC_PACKET)
		printf("qosrcvpkt: entered - Parul \n");
	END_TRACE
#endif
...
int premium_qos_get_link_speed()
{
	uint32_t   speed;
	uint  link_rate;
	int link_error = 0;
	...
	IF_TRACED(TRC_TLVINFO)
       		 printf("Entering: premium_qos_get_link_speed \n");
	END_TRACE
	

	if (!lltd_make_wl_prefix(prefix,sizeof(prefix),0,NULL)){
		IF_TRACED(TRC_TLVINFO)
      		 	printf("get_link_speed, lltd_make_wl_prefix ERROR, exiting!\n");
		END_TRACE
		return -1;
	}
	...
	if ( (link_error = wl_ioctl(name, WLC_GET_RATE, &link_rate, sizeof(link_rate))) < 0){
			...
			IF_TRACED(TRC_TLVINFO)
      		 	printf("get_link_speed, wl_ioctl return error = %d", link_error);
			END_TRACE
	}

	 IF_TRACED(TRC_TLVINFO)
      		 printf("premium_qos_get_link_speed, link_rate = %d", link_rate); 	 
        END_TRACE

		
	//speed = htonl ( (link_rate * 500 * 1000) / 100 );  //in units of 100bps
	speed =  ( (link_rate * 500 * 1000) / 100 );  //in units of 100bps
