 ...
 *
 * $Id: emfc.c 375470 2012-12-18 22:57:18Z $
 */
#include <typedefs.h>
#include <bcmdefs.h>
#include <bcmendian.h>
		...
		switch (action)
		{
			...
			case EMF_CONVERT_QUERY:
			{
					...
					ptr != mgrp->mi_head.prev; ptr = ptr->next)
				{
					...
					// will allocate another skb to hold the modified mac
					// header, so a simple PKTDUP here will be enough.
					if ((sdu_clone = PKTDUP(emfc->osh, sdu)) == NULL)
					{
						OSL_UNLOCK(emfc->fdb_lock);
				...
				if (ifp != mi->mi_mhif->mhif_ifp)
				{
					...
					emfc->wrapper.forward_fn(emfc->emfi, sdu,
								dest_ip, mi->mi_mhif->mhif_ifp, rt_port) ?
								EMFC_STATS_INCR(emfc, mcast_data_dropped) :
								mi->mi_mhif->mhif_data_fwd++,
								mi->mi_data_fwd++;
...
emfc_mfdb_init(emfc_info_t *emfc)
{
	...
	emfc->mc_data_ind=1;  /* at emfc.c#486, it will check this as well, who should assign it? 
	 if this is not assigned, mutlicast traffic will not be passed over to stack*/
#endif
	return;
}
