...
static bool mark_mt_v0(const struct sk_buff *skb, const struct xt_match_param *par)
{
	...
	{
		classid = SKBMARK_GET_TC_ID(skb->mark);
		return (classid>0 && (classid < info->mark));
	}
#endif
	return false;
}

static bool mark_mt(const struct sk_buff *skb, const struct xt_match_param *par)
{
