...
static void ndisc_router_discovery(struct sk_buff *skb)
{
	...
	/* MSTC, Justin Lin, [SPR 121102191] IPv6 LAN doesn't have any entry of "Delegate prefix from WAN" on 6RD DHCP mode  *
         * Merge DSL-491GNU-B1D for Telus r5278, 5785                                                                                                          */
	if (((in6_dev->cnf.forwarding) && 
		(!(in6_dev->dev->priv_flags & IFF_WANDEV) || 
		((in6_dev->dev->priv_flags & IFF_WANDEV) && 
		!strchr(in6_dev->dev->name, '.') && !strstr(in6_dev->dev->name, "atm") && !strstr(in6_dev->dev->name, "ppp"))))
...
#if 1 // __MSTC__, DingRuei  /* accept_ra_defrtr =  Learn default router in Router Advertisement*/
	{
		char cmd[64];
		char defrtaddr[44];
