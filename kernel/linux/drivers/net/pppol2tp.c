...
static int pppol2tp_recv_core(struct sock *sock, struct sk_buff *skb)
{
	...
	sock_put(sock);	//from SBG3300 //__ZyXEL__,bpan,20120410,FIX:stack trace crash when tunnel socket closed
...
static int pppol2tp_xmit(struct ppp_channel *chan, struct sk_buff *skb)
{
	...
	if (sk_tun->sk_no_check == UDP_CSUM_NOXMIT)
		skb->ip_summed = CHECKSUM_NONE;
#if 0	/* ZyXEL 20140521 for PPTP VPN. from SBG3300 */ /*__ZyXEL__,bpan */
	else if (!(skb->dst->dev->features & NETIF_F_V4_CSUM)) {
#else	/*__ZyXEL__,bpan,20120628,FIX kernel crash:
	 * "Unable to handle kernel paging request at virtual address 0000000c"
	 * based on https://bugzilla.redhat.com/show_bug.cgi?id=607054 
	 * (see 2nd "Upstream commit: 
	 *  http://git.kernel.org/?p=linux/kernel/git/torvalds/linux-2.6.git;a=commitdiff;h=3feec909)
	 * but we have to use skb->dst instead of skb_dst(skb) as in the patch
	 * original patch:
			    ...
			    int sockaddr_len, int flags)
{
	...
	if ((sp->pppol2tp.s_session == 0) && (sp->pppol2tp.d_session == 0)) {
		...
		sock_hold(tunnel_sock);	// ZYXEL 20140521 for PPTP VPN from SBG3300 //__ZyXEL__,bpan,20120410,FIX:stack trace crash when tunnel socket closed
