...
#if 1 //Support new flash MACRONIX MX30LF1G08AA 
        {	/* 40 */
		.chipId = MXIC_MX30LF1G08AA,
		.mafId = FLASHTYPE_MXIC,
		.chipIdStr = "MXIC MX30LF1G08AA",
		.options = NAND_USE_FLASH_BBT, 
		.idOptions = 0,
		.timing1 = 0, 
		.timing2 = 0,
...
static int brcmnand_probe(struct mtd_info *mtd, unsigned int chipSelect)
{
	...
	/*------------- 3rd ID byte --------------------*/	
#if !defined(CONFIG_MIPS_BRCM) //<<20140507, TimLiu porting from Broadcom SDK 412L08.
